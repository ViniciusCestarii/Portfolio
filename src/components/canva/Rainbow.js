...
const RainbowMaterial = shaderMaterial(
  ...
  vec3 iridescence(float angle, float thickness) {
    ...
    vec3 hue = mix(physhue2rgb(thickness * interf0 + dp, thickness * phase0),
      physhue2rgb(thickness * interf1 + 0.1 + dp, thickness * phase1), f);
...
export const Rainbow = forwardRef(
  (
    ...
    const length = Math.hypot(width, height) + 20
